...
- [x] F1. Plan compliance audit: PASS — Todo 14 (dyyl streaming host protocol) now IMPLEMENTED: `build_dyyl` spawns real `dyyl --host-json` via `src/dyyl_host.rs`, collects NDJSON mcm_command stream, source_line preserved end-to-end (verified with real dyyl binary), `mcm do file.dyyl` supported (builds to temp then executes); text-parser fallback retained for minimal envs without dyyl binary. Todo 4 (OIDC) remains PARTIAL (nonce skipped at oidc.rs:154-158, JWT signature skipped at oidc.rs:23-25,348-351). Todo 12 Microsoft auth endpoints, launch native extraction, and verify_game_files preflight verified FIXED.
